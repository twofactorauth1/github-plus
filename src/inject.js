    ...
    if (btnGroup && window.location.href && window.location.href.indexOf('blob/' + utils.getBranch()) > -1) {
      ...
      apiUtils.getRepoContent(
        ...
        //   utils.removePrevInstancesOf('.js-file-download');
          ...
          btnGroupHtml = '' +
          '<button aria-label="Copy file contents to clipboard" class="js-file-clipboard btn btn-sm BtnGroup-item file-clipboard-button tooltipped tooltipped-s" data-copied-hint="Copied!" type="button"  data-clipboard-target="tbody">' +
            'Copy File' +
          '</button>';
        ...
        for (var i = 0; i < elems.length; i++) {
          if (data[i].type === 'file') {
            ...
            var html = '<td class="download" style="width: 20px;padding-right: 10px;color: #888;text-align: right;white-space: nowrap;">' +
              ...
              '<a href="' + data[i].download_url + '" title="(Cmd/Ctr + Click) to download File" aria-label="(Cmd/Ctr + Click) to download File" class="tooltipped tooltipped-nw" download="' + data[i].name + '">' +
